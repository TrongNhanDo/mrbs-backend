...
const getAllEntries = (req: Request, res: Response) => {
  try {
    dbConnect.getConnection((err: MysqlError, connection: PoolConnection) => {
      ...
      connection.query(
        query,
        [params.entry_id, params.username, params.create_by, params.registered],
        (error, rows) => {
          if (!error) {
            rows.map((value: any) => {
              console.log({
...
const getEntryByConditions = (req: Request, res: Response) => {
  try {
    dbConnect.getConnection((error: MysqlError, connection: PoolConnection) => {
          ...
          Array(repeatMonth)
            ...
            .map((_, index) => {
              ...
              if (monthRelative) {
                ...
                // target date
                const targetDate = allDays[params.month_relative_ord - 1];
